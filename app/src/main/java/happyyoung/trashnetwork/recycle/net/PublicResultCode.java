package happyyoung.trashnetwork.recycle.net;

/**
 * Created by shengyun-zhou <GGGZ-1101-28@Live.cn> on 2017-02-19
 */
public class PublicResultCode {
    public static final int USER_NOT_EXIST = 200001;
    public static final int INCORRECT_PASSWORD = 200002;

    public static final int USER_NAME_USED = 200011;
    public static final int ILLEGAL_PASSWORD_LENGTH = 200012;
    public static final int ILLEGAL_PASSWORD_FORMAT = 200013;
    public static final int ILLEGAL_EMAIL = 200014;
    public static final int ILLEGAL_ACCOUNT_TYPE = 200015;
    public static final int ILLEGAL_PHONE = 200016;

    public static final int DELIVER_ADDRESS_NOT_FOUND = 200021;

    public static final int CREDIT_RECORD_NOT_FOUND = 200101;
    public static final int CREDIT_RECORD_RECYCLE_POINT_NOT_FOUND = 200111;
    public static final int RECYCLE_POINT_NOT_RECYCLE_BOTTLE = 200112;
    public static final int ILLEGAL_LOCATION = 200113;
    public static final int TOO_FAY_AWAY_FROM_RECYCLE_POINT = 200114;

    public static final int FEEDBACK_NOT_FOUND = 200201;

    public static final int RECYCLE_RECORD_NOT_FOUND = 200401;
    public static final int RECYCLE_RECORD_RECYCLE_POINT_NOT_FOUND = 200402;
    public static final int RECYCLE_POINT_NOT_MANAGED = 200403;
    public static final int RECYCLE_POINT_EMPTY = 200404;

    public static final int EVENT_NOT_FOUND = 200501;

    public static final int COMMODITY_NOT_FOUND = 200601;

    public static final int ORDER_NOT_FOUND = 200621;
}
